<template>
  ...
  >
    ...
    <div class="flex flex-wrap items-center gap-1.5 mb-3">
      ...
      <span class="px-2 py-0.5 text-xs rounded text-white bg-blue-500">IF={{ article.jcrIf }}</span>
      <span v-if="locale === 'zh' && article.cas" class="px-2 py-0.5 text-xs rounded text-white" :class="casColorClass(article.cas)">中科院{{ article.cas }}</span>
    </div>
